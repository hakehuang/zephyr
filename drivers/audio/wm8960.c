...
static int codec_set_output_volume(const struct device *dev, wm8960_module_t module, int volume);
			       ...
			       audio_route_t dai_route)
{
	...
	case AUDIO_ROUTE_PALYBACK:
		...
		break;
	case AUDIO_ROUTE_PACKBACK_CAPTURE:
		/*
             * Left DAC to left output mixer, LINPUT3 left output mixer volume = 0dB
             */
		codec_write_reg(dev, WM8960_LOUTMIX, 0x100);
...
#if 0
    i2c_burst_read(dev_data->i2c_device, dev_cfg->i2c_address,
              reg.reg_addr, val);
#endif
	LOG_DBG("REG:%02u VAL:0x%02x", reg, *val);
}
...
static void WM8960_SetLeftInput(const struct device *dev, wm8960_input_t input)
{
	uint16_t val = 0;
...
static void WM8960_SetRightInput(const struct device *dev, wm8960_input_t input)
{
	uint16_t val = 0;
...
static void WM8960_SetModule(const struct device *dev, wm8960_module_t module,
			     bool isEnabled)
{
	switch (module) {
...
DEVICE_AND_API_INIT(wm8960, DT_INST_LABEL(0), codec_initialize,
