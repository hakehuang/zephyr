...
#define DEV_CFG(dev) ((const struct i2s_mcux_config *const)dev->config)
#define DEV_DATA(dev) ((struct i2s_dev_data *)dev->data)
...
static void i2s_tx_stream_disable(const struct device *dev)
{
	...
	const struct device *dev_dma = DEV_DATA(dev)->dev_dma;
	...
	DEV_CFG(dev)->base->TCR3 &= ~I2S_TCR3_TCE_MASK;

	/* purge buffers queued in the stream */
	i2s_purge_stream_buffers(strm, dev_data->tx.cfg.mem_slab);

	/* Disable DMA enable bit */
	SAI_TxEnableDMA(DEV_CFG(dev)->base, kSAI_FIFORequestDMAEnable,
			false);

	/* Disable Tx */
	SAI_TxEnable(DEV_CFG(dev)->base, false);

	strm->stream_starving = false;

	/* If Tx is disabled, reset the FIFO pointer, clear error flags */
	if ((DEV_CFG(dev)->base->TCSR & I2S_TCSR_TE_MASK) == 0UL) {
		DEV_CFG(dev)->base->TCSR |=
			(I2S_TCSR_FR_MASK | I2S_TCSR_SR_MASK);
		DEV_CFG(dev)->base->TCSR &= ~I2S_TCSR_SR_MASK;
	}
}
...
static void i2s_rx_stream_disable(const struct device *dev)
{
	...
	/* Disable DMA enable bit */
	SAI_RxEnableDMA(DEV_CFG(dev)->base, kSAI_FIFORequestDMAEnable,
			false);

	/* Disable Rx */
	SAI_RxEnable(DEV_CFG(dev)->base, false);
	 ...
	 */
	if ((DEV_CFG(dev)->base->RCSR & I2S_RCSR_RE_MASK) == 0UL) {
		DEV_CFG(dev)->base->RCSR |=
			(I2S_RCSR_FR_MASK | I2S_RCSR_SR_MASK);
		DEV_CFG(dev)->base->RCSR &= ~I2S_RCSR_SR_MASK;
	}
}
				...
				void *arg, uint32_t channel, int status)
{
	...
	case I2S_STATE_RUNNING:
		...
		if (ret == 0) {
			/* config the DMA */
			uint32_t data_path = DEV_DATA(dev)->tx.start_channel;

			strm->dma_block.block_size = strm->cfg.block_size;
			strm->dma_block.dest_address =
				(uint32_t)&DEV_BASE(dev)->TDR[data_path];
...
{
	const struct device *iomuxgpr_dev = DEV_CFG(dev)->pinmux_dev;
	uint32_t offset = 0;
	uint32_t mask = 0;
	...
	case 0:
#ifdef CONFIG_I2S_0
		offset = DT_PHA_BY_IDX(DT_DRV_INST(0), pinmuxs, 0, pin);
		mask = DT_PHA_BY_IDX(DT_DRV_INST(0), pinmuxs, 0, function);
#endif
		break;
	case 1:
#ifdef CONFIG_I2S_1
		offset = DT_PHA_BY_IDX(DT_DRV_INST(1), pinmuxs, 0, pin);
		mask = DT_PHA_BY_IDX(DT_DRV_INST(1), pinmuxs, 0, function);
#endif
		break;
	case 2:
#ifdef CONFIG_I2S_2
		offset = DT_PHA_BY_IDX(DT_DRV_INST(2), pinmuxs, 0, pin);
		mask = DT_PHA_BY_IDX(DT_DRV_INST(2), pinmuxs, 0, function);
#endif
	default:
...
{
	const struct i2s_mcux_config *const dev_cfg = DEV_CFG(dev);
	...
	case 0:
#ifdef CONFIG_I2S_0
		ccm_dev = DEVICE_DT_GET(DT_CLOCKS_CTLR(DT_DRV_INST(0)));
		clk_sub_sys = (clock_control_subsys_t)DT_CLOCKS_CELL_BY_IDX(
			DT_DRV_INST(0), 0, name);
		src_div = DT_CLOCKS_CELL_BY_IDX(DT_DRV_INST(0), 2, bits);
		pre_div = DT_CLOCKS_CELL_BY_IDX(DT_DRV_INST(0), 1, bits);
#endif
		break;
	case 1:
#ifdef CONFIG_I2S_1
		ccm_dev = DEVICE_DT_GET(DT_CLOCKS_CTLR(DT_DRV_INST(1)));
		clk_sub_sys = (clock_control_subsys_t)DT_CLOCKS_CELL_BY_IDX(
			DT_DRV_INST(1), 0, name);
		src_div = DT_CLOCKS_CELL_BY_IDX(DT_DRV_INST(1), 2, bits);
		pre_div = DT_CLOCKS_CELL_BY_IDX(DT_DRV_INST(1), 1, bits);
#endif
		break;
	case 2:
#ifdef CONFIG_I2S_2
		ccm_dev = DEVICE_DT_GET(DT_CLOCKS_CTLR(DT_DRV_INST(2)));
		clk_sub_sys = (clock_control_subsys_t)DT_CLOCKS_CELL_BY_IDX(
			DT_DRV_INST(2), 0, name);
		src_div = DT_CLOCKS_CELL_BY_IDX(DT_DRV_INST(2), 2, bits);
		pre_div = DT_CLOCKS_CELL_BY_IDX(DT_DRV_INST(2), 1, bits);
#endif
	default:
		LOG_ERR("not a valid i2s instance");
		*mclk = rate;
		return;
	}
	if (ccm_dev != NULL) {
			   ...
			   const struct i2s_config *i2s_cfg)
{
	...
	if (dir == I2S_DIR_TX) {
		...
		SAI_TxSetConfig(DEV_BASE(dev), &config);
		dev_data->tx.start_channel = config.startChannel;
		/* Disable the channel FIFO */
		DEV_BASE(dev)->TCR3 &= ~I2S_TCR3_TCE_MASK;
		...
		/* set bit clock divider */
		SAI_RxSetConfig(DEV_BASE(dev), &config);
...
static int i2s_tx_stream_start(const struct device *dev)
{
	...
	SAI_TxEnableDMA(DEV_CFG(dev)->base, kSAI_FIFORequestDMAEnable, true);

	/* Enable SAI Tx clock */
	SAI_TxEnable(DEV_CFG(dev)->base, true);

	/* Enable the channel FIFO */
	DEV_CFG(dev)->base->TCR3 |= I2S_TCR3_TCE(1UL << strm->start_channel);
...
static int i2s_rx_stream_start(const struct device *dev)
{
	...
	/* Enable the channel FIFO */
	DEV_CFG(dev)->base->RCR3 |= I2S_RCR3_RCE(1UL << strm->start_channel);

	/* Enable SAI Rx clock */
	SAI_RxEnable(DEV_CFG(dev)->base, true);
...
static void sai_driver_irq(const struct device *dev)
{
	if ((DEV_BASE(dev)->TCSR & I2S_TCSR_FEF_MASK) == I2S_TCSR_FEF_MASK) {
		...
		SAI_TxClearStatusFlags(DEV_BASE(dev), I2S_TCSR_FEF_MASK);

		/* Reset FIFO for safety */
		SAI_TxSoftwareReset(DEV_BASE(dev), kSAI_ResetTypeFIFO);

		LOG_DBG("sai tx error occurred");
	}

	if ((DEV_BASE(dev)->RCSR & I2S_RCSR_FEF_MASK) == I2S_RCSR_FEF_MASK) {
		/* Clear FIFO error flag to continue transfer */
		SAI_RxClearStatusFlags(DEV_BASE(dev), I2S_RCSR_FEF_MASK);

		/* Reset FIFO for safety */
		SAI_RxSoftwareReset(DEV_BASE(dev), kSAI_ResetTypeFIFO);
...
static void i2s_mcux_isr(void *arg)
{
	...
	if ((DEV_BASE(dev)->RCSR & I2S_TCSR_FEF_MASK) == I2S_TCSR_FEF_MASK) {
		sai_driver_irq(dev);
	}
...
{
	uint32_t lp, pd, num, den, src, clK_src, pre_div, src_div;
	clock_audio_pll_config_t audioPllConfig;

	if (DEV_CFG(dev)->bus_id == 0) {
#ifdef CONFIG_I2S_0
		src = DT_PHA_BY_IDX(DT_DRV_INST(0), pll_clocks, 0, bits);
		lp = DT_PHA_BY_IDX(DT_DRV_INST(0), pll_clocks, 1, bits);
		pd = DT_PHA_BY_IDX(DT_DRV_INST(0), pll_clocks, 2, bits);
		num = DT_PHA_BY_IDX(DT_DRV_INST(0), pll_clocks, 3, bits);
		den = DT_PHA_BY_IDX(DT_DRV_INST(0), pll_clocks, 4, bits);
		clK_src = DT_CLOCKS_CELL_BY_IDX(DT_DRV_INST(0), 0, bits);
		pre_div = DT_CLOCKS_CELL_BY_IDX(DT_DRV_INST(0), 1, bits);
		src_div = DT_CLOCKS_CELL_BY_IDX(DT_DRV_INST(0), 2, bits);
...
#ifdef CONFIG_I2S_1
		src = DT_PHA_BY_IDX(DT_DRV_INST(1), pll_clocks, 0, bits);
		lp = DT_PHA_BY_IDX(DT_DRV_INST(1), pll_clocks, 1, bits);
		pd = DT_PHA_BY_IDX(DT_DRV_INST(1), pll_clocks, 2, bits);
		num = DT_PHA_BY_IDX(DT_DRV_INST(1), pll_clocks, 3, bits);
		den = DT_PHA_BY_IDX(DT_DRV_INST(1), pll_clocks, 4, bits);
		clK_src = DT_CLOCKS_CELL_BY_IDX(DT_DRV_INST(1), 0, bits);
		pre_div = DT_CLOCKS_CELL_BY_IDX(DT_DRV_INST(1), 1, bits);
		src_div = DT_CLOCKS_CELL_BY_IDX(DT_DRV_INST(1), 2, bits);
...
#ifdef CONFIG_I2S_2
		src = DT_PHA_BY_IDX(DT_DRV_INST(2), pll_clocks, 0, bits);
		lp = DT_PHA_BY_IDX(DT_DRV_INST(2), pll_clocks, 1, bits);
		pd = DT_PHA_BY_IDX(DT_DRV_INST(2), pll_clocks, 2, bits);
		num = DT_PHA_BY_IDX(DT_DRV_INST(2), pll_clocks, 3, bits);
...
#define I2S_MCUX_INIT(i2s_id)						\
	...
	static const struct i2s_mcux_config i2s_##i2s_id##_config = {	\
		...
		.tx_sync_mode =						\
		  UTIL_AND(DT_INST_NODE_HAS_PROP(			\
			   i2s_id, nxp_tx_sync_mode),			\
			   DT_INST_PROP(i2s_id, nxp_tx_sync_mode)),	\
		.rx_sync_mode =						\
		  UTIL_AND(DT_INST_NODE_HAS_PROP(			\
			   i2s_id, nxp_rx_sync_mode),			\
