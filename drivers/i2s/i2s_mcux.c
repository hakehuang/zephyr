...
static void i2s_tx_stream_disable(const struct device *dev)
{
	...
	SAI_TxEnableDMA(DEV_CFG(dev)->base, kSAI_FIFORequestDMAEnable, false);
...
static void enable_mclk_direction(const struct device *dev, bool dir)
{
	...
	if (iomuxgpr_dev == NULL) {
		LOG_ERR("fail to find pinmux controller %s\n",
			DEV_CFG(dev)->pinmux_name);
		return;
	}
...
static void get_mclk_rate(const struct device *dev, uint32_t *mclk)
{
	...
	case 0:
		...
		ccm_dev = device_get_binding(
			DT_CLOCKS_LABEL_BY_IDX(DT_DRV_INST(0), 0));
...
#ifdef CONFIG_I2S_1
		ccm_dev = device_get_binding(
			DT_CLOCKS_LABEL_BY_IDX(DT_DRV_INST(1), 0));
		clk_sub_sys = (clock_control_subsys_t)DT_CLOCKS_CELL_BY_IDX(
...
#ifdef CONFIG_I2S_2
		ccm_dev = device_get_binding(
			DT_CLOCKS_LABEL_BY_IDX(DT_DRV_INST(2), 0));
		clk_sub_sys = (clock_control_subsys_t)DT_CLOCKS_CELL_BY_IDX(
			   ...
			   struct i2s_config *i2s_cfg)
{
	...
	if (dir == I2S_DIR_TX) {
		...
		LOG_DBG("tx slab num_blocks = 0x%d",
		...
		LOG_DBG("rx slab free_list = 0x%x",
			(uint32_t)i2s_cfg->mem_slab->free_list);
		LOG_DBG("rx slab num_blocks = 0x%d",
			(uint32_t)i2s_cfg->mem_slab->num_blocks);
...
struct i2s_config *i2s_mcux_config_get(const struct device *dev,
				       enum i2s_dir dir)
{
	struct i2s_dev_data *dev_data = DEV_DATA(dev);
