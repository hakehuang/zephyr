...
static int mcux_adc16_init(const struct device *dev)
{
	...
	if (config->periodic_trigger) {
		data->adc_dma_config.dma_channel =
			dma_request_channel(data->dev_dma, DMA_CHANNEL_PERIODIC);
	} else {
		data->adc_dma_config.dma_channel =
			dma_request_channel(data->dev_dma, DMA_CHANNEL_NORMAL);
	}
...
#define ACD16_MCUX_INIT(n)						\
	...
	{								\
		IRQ_CONNECT(DT_INST_IRQN(n), DT_INST_IRQ(n, priority),	\
			    mcux_adc16_isr, DEVICE_DT_INST_GET(n), 0);	\
									\
